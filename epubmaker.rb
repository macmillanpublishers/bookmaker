...
File.open("#{tmp_dir}\\#{tmp_id}\\epub_tmp.html", 'w') do |output| 
	output.write filecontents
end

# convert to epub
`java -jar C:\\saxon\\saxon9pe.jar -s:#{tmp_dir}\\#{tmp_id}\\epub_tmp.html -xsl:S:\\resources\\HTMLBook\\HTMLBook-master\\htmlbook-xsl\\epub.xsl -o:#{tmp_dir}\\#{tmp_id}\\tmp.epub`

# fix cover.html doctype
covercontents = File.read("#{tmp_dir}\\#{tmp_id}\\OEBPS\\cover.html")
replace = covercontents.gsub(/&lt;!DOCTYPE html&gt;/,"<!DOCTYPE html>")
File.open("#{tmp_dir}\\#{tmp_id}\\OEBPS\\cover.html", "w") {|file| file.puts replace}

# fix author info in opf
covercontents = File.read("#{tmp_dir}\\#{tmp_id}\\OEBPS\\content.opf")
replace = covercontents.gsub(/<dc:creator/,"<dc:identifier id='isbn'>#{eisbn}</dc:identifier><dc:creator role='aut'")
File.open("#{tmp_dir}\\#{tmp_id}\\OEBPS\\content.opf", "w") {|file| file.puts replace}

# add epub css to epub folder
`copy #{working_dir}\\done\\#{pisbn}\\layout\\epub.css #{tmp_dir}\\#{tmp_id}\\OEBPS\\`

# add cover image file to epub folder
`copy #{working_dir}\\done\\#{pisbn}\\cover\\cover.jpg #{tmp_dir}\\#{tmp_id}\\OEBPS\\`

#copy tor logo image file to epub folder
`copy #{working_dir}\\resources\\torDOTcom\\img\\torlogo.jpg #{tmp_dir}\\#{tmp_id}\\OEBPS\\`

# zip epub
`C:\\zip\\zip.exe #{eisbn}_EPUB.epub -DX0 #{tmp_dir}\\#{tmp_id}\\mimetype`
`C:\\zip\\zip.exe #{eisbn}_EPUB.epub -rDX9 #{tmp_dir}\\#{tmp_id}\\META-INF #{tmp_dir}\\#{tmp_id}\\OEBPS`
...
`del #{tmp_dir}\\#{tmp_id}\\epub_tmp.html`